...
<body>
	<%
		...
		String user_id=session.getAttribute("user_id").toString();//获取url传过来的UserId
	        ...
	        List<Object> userList = user.getEntityById(user_id);
	        List<Object> seatList=seat.getEntityById(seat_id);//根据ID获取Seat数据
	        Object[] ob =(Object[])userList.get(0);
	        Object[] obj=(Object[])seatList.get(0);//将List数据转换成Object[]
	        		
	        		if(ob[2].toString().equals("null")){
	        			
	        			request.getRequestDispatcher("/Admin/CardAdd.jsp").forward(request, response);
	        		}
	 ...
	 <form action="<%=path %>/SeatHandle?type=2" method="post">
		<table style=" margin:50px auto;">
	            <tbody>
			 <tr><td>车位ID：</td><td><input type="text" name="seat_id" value=<%=ob[0].toString()%> readonly /></td></tr>
