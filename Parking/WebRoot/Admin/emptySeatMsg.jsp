...
<script>
	$(function()
	{
		$(".btnGo").click(function()
		{
			if($("[name=htype]").val()=="4")
			{
				location.href="<%=path %>/SeatHandle?type=9&page="+($(".txtPage").val()==""?1:$(".txtPage").val());
			}
			else
			{
				...
				out.write("<input type='hidden' name='hvalue' value='"+(request.getAttribute("value")!=null?request.getAttribute("value").toString():"")+"' />");
                		if(list!=null) //循环数据列表，生成表格行
                		{
                			for(int i=0;i<list.size();i++)
	                		{
	               				...
	               				out.print("<tr><td>"+obj[0]+"</td><td>"+obj[1]+"</td><td>"+obj[2]+"</td><td>"+result+"</td><td>"+obj[4]+"</td><td><a href='"+path+"/Admin/GetSeat.jsp?seat_id="+obj[0]+"' class='a_edit'>预订</a></td></tr>");
	                		}
