		...
		public List<Object> getUsedSeat()
		{
			String sqlCmd="SELECT *FROM Seat WHERE seat_id  IN(SELECT seat_id FROM card)";
			return DBUtil.SQLUtil.executeQuery(sqlCmd, null);//执行查询操作executeQuery
		}
		...
		public List<Object> getEntityByid(String user_id)
		{
			...
			String sqlCmd="select * from seat where seat_id  in (select seat_id from card where user_name in (select " + "real_name from user where  user_id='"+user_id+"'));";
        ...
        public int updateEntityByid(String seat_id)
        {
            String sqlCmd="Update Seat set seat_state='1" +   "' where seat_id='"+seat_id+"'";
            return SQLUtil.executeNonQuery(sqlCmd, null);
        }
