...
public class Charge {
	       ...
	       //获取分页后用户表信息列表
	        ...
	        {
	            String sqlCmd="select *From updatecharge where user_id='"+user_id+"'";
	            return DBUtil.SQLUtil.executeQuery(sqlCmd, null);//执行查询操作executeQuery
	        }
			...
			public List<Object> getChargeOfHour(String user_id)
			{
				String sqlCmd="select  money_ber_hours from chargeofhours where seat_section in(select  seat_section from seat  where  seat_id in(select seat_id from card where user_name in (select real_name from user where user_id = '"+user_id+"')))";
