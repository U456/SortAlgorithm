...
import java.util.*;
			...
			private void getcharge() throws IOException ,ServletException {
				
				DAL.Charge charge = new DAL.Charge();
				response.setCharacterEncoding("UTF-8");
				response.setContentType("text/html;charset=UTF-8");
				PrintWriter out=response.getWriter();
				HttpSession session=request.getSession();
				String userId=session.getAttribute("user_id").toString();
				List<Object> list=charge.getEntityById(userId);//获取数据列表
				List<Object> mm = charge.getChargeOfHour(userId);
			
				request.setAttribute("list",list);//将数据存放到request对象中，用于转发给前台页面使用
				request.setAttribute("bermy",mm);//将数据存放到request对象中，用于转发给前台页面使用
				request.getRequestDispatcher("/Admin/usercharge.jsp").forward(request, response);//请求转发
			...
			private void updateEntity() throws UnsupportedEncodingException
			{
				...
				String gander=new String(request.getParameter("gander").getBytes("ISO8859_1"),"UTF-8");

				if(user.updateEntity(user_id,user_name,real_name,user_pwd,user_phone,age,gander)==1)
				{
					try {
							if(request.getSession().getAttribute("role_id").toString().equals("r001"))
							{
								response.sendRedirect("/Parking/Index.jsp");//成功更新数据后跳转至UserInfo.jsp页面
							}
							else {
								response.sendRedirect("/Parking/Common/UserInfoMsg.jsp");//成功更新数据后跳转至UserInfo.jsp页面
							}
					} catch (IOException e) {
